...
int ft_traceray(t_list_minirt *camera, t_float3 direction, float tmin, float tmax, t_list_minirt *head, t_color *background_color)
{
	...
	closest_t = ft_closest_intersection(ft_get_camera_position(camera),direction,tmin,tmax,head, &closest_sphere);
	...
	tmp = ft_float_times_t_color(ft_get_sphere_color(closest_sphere), ft_compute_lighting(p,n,head,tmin, tmax));
