...
float ft_closest_intersection(t_float3 origin, t_float3 direction, float tmin, float tmax, t_list_minirt *head, t_list_minirt **closest_shape)
{
	...
	while ((current_shape = ft_find_next_shape(current_shape, head)))
	{
		if (ft_is_a_sphere(current_shape))
			...
			solutions = ft_intersect_ray_with_plane(origin, direction, ft_get_shape_center(current_shape), ft_get_shape_normal(current_shape));
		else if (ft_is_a_cylinder(current_shape))
			solutions = ft_intersect_ray_with_cylinder(origin, direction, current_shape);
